...
## [1.40.1](https://github.com/atanuroy22/morphe-patches/compare/v1.40.0...v1.40.1) (2026-08-25)

Fork changes on top of upstream `v1.40.0`:

### 🔧 Fork defaults re-applied after the upstream merge

* **YouTube - Feed:** `Hide 'Notify me' button` stays off by default (upstream changed it to on).
* **YouTube - Swipe controls:** Upstream replaced the per-gesture toggles with swipe zone actions.
  The fork now ships `Left zone = Brightness`, `Right zone = Volume`, `Swipe to change videos = on`
  and `Auto-brightness at lowest value = on`.
* **YouTube - General:** `morphe_music_package_name` was renamed upstream to
  `morphe_custom_music_package_name`; the fork default of `app.revanced.android.apps.youtube.music`
  is re-applied to the new key.
* **YouTube - Branding:** Header logo entry 4 is displayed as `Premium`, and the new
  `Add to <app> queue` flyout string uses `Premium Youtube`.

### 🗑️ Fork removals

* **YouTube - Settings:** The `About` entry at the top of `Extra Settings` is no longer added.
